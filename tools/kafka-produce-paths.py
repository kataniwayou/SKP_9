#!/usr/bin/env python3
"""
Seeds the dev Kafka topic that Processor.PathImporter reads.

A record on this topic IS a file path -- no envelope, no JSON, nothing to deserialize. The
processor's PathRecord says so explicitly, and the branch data it opens per path is
{"path": "<the record value>"}. So this producer writes bare UTF-8 strings and nothing else;
adding a wrapper here would silently change what every downstream step receives.
...
Usage:
    ...
    python tools/kafka-produce-paths.py --file paths.txt
    python tools/kafka-produce-paths.py --count 3 --prefix /mnt/incoming/batch-07
"""
