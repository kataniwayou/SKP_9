...
**4. Names no longer depend on a workflow having run.** They used to: the pairs came from records
`OrchestrationService` writes at start time, and the cron does **not** come through that path — it
fires from the orchestrator against an ids-only projection — so a cron-only workflow had no names at
all, and one whose last start had aged out of the index reverted to raw GUIDs. The BaseApi now
publishes the table from the entity tables instead, so a step is nameable the moment it is
published, run or not.

Nothing is regenerated by hand. A dashboard render fetches `lookup/ping.svg`, and that republishes
the map. The cost is that the dashboard is **one render behind**: Kibana loads the data view before
the panels render, so a freshly published entity shows as a GUID until the next refresh.

An unmapped id always renders as **itself**, never blank — the formatters omit `unknownKeyValue`
precisely so that two unlabelled steps cannot collapse into one legend bucket.
