...
public sealed class ArchiveExpanderLocatorTests
{
    ...
    private static (ArchiveExpanderProcessor Processor, RecordingLogger<ArchiveExpanderProcessor> Log) Build()
    {
        ...
        var processor = new ArchiveExpanderProcessor(
            log, Options.Create(new ArchiveExpanderOptions()), new FileContentBuilder([]));
        processor.BeginDispatch(new DispatchState(Substitute.For<IQueueSender>(), C, W, S, P));
        return (processor, log);
    }

    private static Task Run(ArchiveExpanderProcessor processor, string json)
