...
public sealed class ArchiveExpanderGuardTests : IDisposable
{
    ...
    private readonly string _dir = Directory.CreateTempSubdirectory("skp-archiveexpander-").FullName;
    ...
    private static (ArchiveExpanderProcessor Processor, RecordingLogger<ArchiveExpanderProcessor> Log) Build(
        long podCeiling = 33_554_432)
    {
        var log = new RecordingLogger<ArchiveExpanderProcessor>();
        var processor = new ArchiveExpanderProcessor(
            log, Options.Create(new ArchiveExpanderOptions { MaxFileSizeBytes = podCeiling }),
    ...
    private static Task Run(ArchiveExpanderProcessor processor, string path, string payload)
