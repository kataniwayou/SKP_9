...
{
    private const string Address = "skp:11111111-1111-1111-1111-111111111111:cache:chain-artists";

    private static (RedisFieldWhitelist Whitelist, RecordingLogger<RedisFieldWhitelist> Log)
        Build(InMemoryL2 l2, string address = Address)
    {
        var log = new RecordingLogger<RedisFieldWhitelist>();
        return (new RedisFieldWhitelist(l2.Multiplexer, address, log), log);
    }
    ...
    {
        // The root is what tells two whitelists apart on one board. Logging the full address would
        // put a workflow id in every slice label and make two runs of the same workflow — or the
        // same list under two workflows — look like different lists.
        var (whitelist, log) = Build(await Projected("SKP Live Suite"));
