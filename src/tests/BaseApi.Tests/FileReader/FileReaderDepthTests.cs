...
/// </para>
/// </summary>
public sealed class FileReaderDepthTests : IDisposable
{
    ...
    private readonly string _dir = Directory.CreateTempSubdirectory("skp-filereader-depth-").FullName;
    ...
    private static (FileReaderProcessor Processor, IQueueSender Sender, RecordingLogger<FileReaderProcessor> Log) Build()
    {
    ...
    public async Task ADepthOutsideTheSupportedRangeIsARejectedPayload()
    {
        ...
        foreach (var depth in new[] { 0, -1, FileReaderConfig.MaxSupportedDepth + 1 })
        {
