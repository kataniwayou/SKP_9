...
/// <summary>
/// The adapter, against a real broker. Everything else in the PathImporter suite runs through
/// <c>FakePathConsumer</c>, which is what keeps the hermetic run broker-free — but it also means
/// <see cref="KafkaPathConsumer"/> itself, and every assumption it makes about how librdkafka
...
public sealed class PathImporterLiveTests
{
    /// <summary>
    ...
    public void DoesNotClaimAnAssignmentWhileTheBrokerIsUnreachable()
    {
        ...
        using var consumer = new KafkaPathConsumer(RealStack.KafkaBrokers, $"live-assign-{Guid.NewGuid():N}");
        ...
        // to get assigned and a poll can return a path, which it keeps rather than drops; the loop
    ...
    /// </summary>
    private static bool ClaimsAssignment(KafkaPathConsumer consumer, TimeSpan timeout)
    {
        try
        {
