...
public static class ProcessorHost
{
        ...
        string[] args, ProcessorIdentityFound identity, Action<IConfigurationBuilder>? configure = null)
    {
        ...
        // this would come up healthy, go Ready, and fail nothing until a dispatch arrived — where a
        // consumer with nowhere to connect finds no records and the step reports Drained, the one
        // terminal that means "the topic is empty". Throwing here makes the same mistake a pod that
        // will not start, which is a thing an operator can see.
        ...
        builder.Services.AddSingleton<IRecordConsumerFactory, KafkaRecordConsumerFactory>();
        ...
        builder.Services.AddSingleton<BaseProcessor.Core.Processing.BaseProcessor, KafkaImporterProcessor>();
